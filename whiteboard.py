...
def consecutiveSums(X):
    ...
    result = 0
    doubleX= 2*X
    for n in range(2, int(sqrt(doubleX))+1):  # 范围限定,必须至少2个数,而最多只除到sqrt(2X),因为后面就是重复的, 因为A0AN 必须 > n
        if doubleX % n == 0:  # 也就是能整除
            A0AN = doubleX // n # 存在一个n项数列,收尾相加等于A0AN
            # 这里必须要讨论奇偶性:
            if n % 2 == 0 and A0AN % 2 != 0: # 偶数项数列, 收尾相加必不可能是偶数
                result += 1
            # elif n % 2 != 0 and A0AN % 2 == 0: # 奇数项数列, 收尾相加必是偶数
