...
class Solution:
    def validPalindrome(self, s):  # time limit exceeded
        if s == s[::-1]:
            return True
        else:
    ...
    def validPalindrome(self, s):  # use string slice combination, still exceed time limit
        if s == s[::-1]:
            return True
        for i in range(len(s)):
            s_new = s[:i] + s[i + 1:]
    ...
    def validPalindrome(self, s):  # from two ends, check step by step
