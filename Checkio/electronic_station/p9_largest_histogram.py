...
def largest_histogram2(histogram):
    ...
    while n < index:
        for m in range(0, n + 1):
            gp = histogram[m:index - n + m]
            result.append(min(gp)*len(gp))
        else:
            n += 1
    return max(result)
