...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current = self.place
        num_spaces = 0
        while current != None and type(current) != Hive:
            closest = random_or_none(current.bees)
            if closest != None and num_spaces <= self.max_range and num_spaces >= self.min_range:
                return closest
            else:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        if self.digesting <= 0:
            rand_bee = random_or_none(self.place.bees)
            if rand_bee != None:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            def get_behind(place):
                if place == None:
                    return []
                return [place.exit] + get_behind(place.exit)
            places = get_behind(self.place)
            ants_to_double = [p.ant for p in places if p != None]
            for ant in ants_to_double:
                if ant != None:
                    if ant not in self.doubled_ants:
                        ant.damage = 2 * ant.damage
                        self.doubled_ants.append(ant)
                    if ant.container:
                        if ant.ant != None and ant.ant not in self.doubled_ants:
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.armor - amount <= 0 and not self.is_imposter:
            bees_win()
        else:
...
def apply_effect(effect, bee, duration):
    ...
    new_action = effect(previous_action)
    def new_action_wrapper(colony):
        nonlocal duration
        nonlocal new_action
        # print(duration)
        if duration > 0:
